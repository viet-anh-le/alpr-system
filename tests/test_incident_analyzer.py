...
@pytest.mark.unit
def test_incident_analyzer_uses_async_pipeline_by_default():
    """Incident analyzer should use the same fast pipeline as upload jobs."""
    from api.core import incident_analyzer
    from api.core.pipeline_async import process_frames_async

    assert incident_analyzer.process_frames is process_frames_async
...
@pytest.mark.unit
def test_run_incident_emits_error_event_on_exception(monkeypatch):
    """Lines 192-214: run_incident error path when process_frames raises."""
    from api.core import incident_analyzer
    ...
    monkeypatch.setattr(incident_analyzer, "process_frames", exploding_process)
    monkeypatch.setattr(incident_analyzer, "_persist_incident", lambda *a, **kw: None)
    ...
    error_events = [e for e in events if e["type"] == "incident_error"]
    assert len(error_events) == 1
    assert "GPU exploded" in error_events[0]["message"]
    assert error_events[0]["incident_id"] == "inc_err"


@pytest.mark.unit
def test_run_incident_accepts_monitor_ocr_backend(monkeypatch):
...
@pytest.mark.unit
def test_run_incident_passes_timings_when_debug_enabled(monkeypatch, caplog):
    from api.core import incident_analyzer
    ...
    monkeypatch.setattr(incident_analyzer, "ALPR_DEBUG_TIMINGS", True)
    monkeypatch.setattr(incident_analyzer, "process_frames", fake_process_frames)
    monkeypatch.setattr(incident_analyzer, "_persist_incident", lambda *a, **kw: None)
    caplog.set_level("INFO", logger="api.core.incident_analyzer")

    try:
        incident_analyzer.run_incident(
            incident_id="inc_timing",
    ...
    assert "Incident timings incident=inc_timing" in caplog.text


@pytest.mark.unit
def test_persist_incident_builds_incident_document_and_calls_upsert(monkeypatch):
    """Lines 48-114: _persist_incident when DB is configured — builds IncidentVehicle docs."""
    ...
    async def fake_upsert(incident):
        upserted.append(incident)

    monkeypatch.setattr(mongodb_mod, "upsert_incident", fake_upsert)
    ...
    incident_analyzer._persist_incident(
        incident_id="inc_db_test",
    ...
    assert upserted[0].incident_id == "inc_db_test"
