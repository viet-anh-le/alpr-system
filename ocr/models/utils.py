...
def accuracy(logits, labels, lengths, chars, epoch=None):
    ...
    for i, length in enumerate(lengths):
        ...
        # Calculate exactly like the original code:
        is_correct = np.array_equal(np.array(pred_labels[i]), label.cpu().numpy())
        if is_correct:
            TP += 1

        if not is_correct and np.random.rand() < 0.02:
            pred_str = "".join([chars[int(c)] for c in pred_labels[i]])
            gt_str = "".join([chars[int(c)] for c in label.cpu().numpy()])

            epoch_str = f"Epoch {epoch}" if epoch is not None else "Unknown Epoch"
            log_msg = f"[{epoch_str}] Pred: '{pred_str}'  VS  GT: '{gt_str}'\n"

            with open("mismatch_log.txt", "a", encoding="utf-8") as f:
                f.write(log_msg)
