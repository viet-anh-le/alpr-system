...
export default function MediaStage({
    ...
    const activePreviewFrame = showingProcessed
        ? preprocessedPreviewFrame
        : previewFrame;
    const showPreviewFrame = !!activePreviewFrame?.b64 && !isDone;
    const displayVideoUrl = showingProcessed
    ...
    return (
        <section className="surface-panel overflow-hidden">
            <div className="panel-header">
                <div>
                    <p className="section-label">Dữ liệu hình ảnh</p>
                </div>
                    ...
                    {!displayVideoUrl && !showPreviewFrame && (
                        <div className="p-4">
                            <EmptyState title="Chưa có nguồn phân tích">
                                Chọn video tải lên để xem khung hình phân tích
                                và tiến trình OCR tại đây.
                            </EmptyState>
                        </div>
                    )}
...
function OverlayBox({ box, imageWidth }) {
    ...
    const label =
        box.label || `${box.cls || "vehicle"} #${box.id ?? ""}`.trim();
