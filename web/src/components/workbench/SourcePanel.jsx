...
import { OCR_OPTIONS, PREPROCESS_OPTIONS, formatBytes } from './constants'

const SOURCE_OPTIONS = [
  { value: 'video', label: 'Video upload' },
  { value: 'camera', label: 'Camera clip' },
  { value: 'image', label: 'Ảnh tĩnh', disabled: true },
]
...
export default function SourcePanel({
  ...
  const startRecording = async () => {
    ...
    recorder.onstop = () => {
      ...
      setFile(new File([blob], `camera-clip-${Date.now()}.webm`, { type: 'video/webm' }))
      setRecording(false)
    }
    recorder.start()
    setRecording(true)
    ...
    <section className="surface-panel overflow-hidden">
      <div className="panel-header">
        <div>
          ...
          <h2 className="mt-1 text-lg font-bold">Chọn video để phân tích</h2>
        </div>
      ...
      <div className="space-y-4 p-4">
        {!compact && (
          <SegmentedControl value={source} onChange={setSource} options={SOURCE_OPTIONS} className="w-full justify-start" />
        )}
        ...
        {source === 'video' && !compact && (
          ...
          >
            ...
            <p className="font-semibold">Kéo video vào đây hoặc chọn file</p>
            <p className="mt-1 text-sm text-[var(--color-text-muted)]">MP4, WebM, MOV, AVI, MKV. Ảnh tĩnh cần endpoint `/upload/image` ở phase sau.</p>
          </div>
        )}
        ...
        {source === 'camera' && !compact && (
          <div className="rounded-[var(--radius-panel)] border border-[var(--color-border)] bg-black/20 p-3">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <Button size="sm" onClick={startCamera}>Mở camera</Button>
              {!recording ? (
                <Button size="sm" variant="primary" onClick={startRecording}>Ghi clip WebM</Button>
              ) : (
                <Button size="sm" variant="danger" onClick={stopRecording}>Dừng ghi</Button>
              )}
              <Button size="sm" variant="ghost" onClick={stopCamera}>Tắt camera</Button>
            </div>
          </div>
        )}
        ...
        {source === 'image' && !compact && (
          <EmptyState title="Ảnh tĩnh chưa được nối backend">
            Phase này giữ API hiện có. Khi thêm `POST /upload/image`, tab này sẽ dùng cùng kết quả evidence workbench.
          </EmptyState>
        )}
          ...
          <div className="rounded-[var(--radius-panel)] border border-[var(--color-border)] bg-[var(--color-bg-elevated)] p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-xs text-[var(--color-text-subtle)]">{formatBytes(file.size)} · {file.type || 'video file'}</p>
              </div>
              <Button variant="primary" loading={loading} disabled={disabled || !file} onClick={start}>
                Chạy nhận dạng
              </Button>
            </div>
          </div>
        ...
        <div className="rounded-[var(--radius-panel)] border border-[var(--color-border)] bg-black/10">
          {compact ? (
            ...
            >
              Thiết lập mô hình
              <span className="data-font text-xs">{advancedOpen ? 'ẩn' : 'mở'}</span>
            </button>
          )}
          {advancedOpen && !compact && (
            <div className="grid gap-3 border-t border-[var(--color-border)] p-4 md:grid-cols-2">
              <Select
                ...
                onChange={(event) => onPreprocessModeChange(event.target.value)}
                options={PREPROCESS_OPTIONS}
              />
              <Select
                label="OCR backend"
                value={ocrBackend}
                onChange={(event) => onOcrBackendChange(event.target.value)}
                options={OCR_OPTIONS}
              />
            </div>
          )}
          {compact && (
            <div className="border-t border-[var(--color-border)] px-4 py-3 text-xs text-[var(--color-text-muted)]">
              {PREPROCESS_OPTIONS.find((option) => option.value === preprocessMode)?.label || preprocessMode}
              {' · '}
              {OCR_OPTIONS.find((option) => option.value === ocrBackend)?.label || ocrBackend}
            </div>
          )}
        </div>
      </div>
