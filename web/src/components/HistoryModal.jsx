...
export default function HistoryModal({ open, onClose }) {
        ...
        async function fetchVehicles() {
            ...
            try {
                const data = await apiJson(
                    `/sessions/${selectedJobId}/records`,
                );
    ...
    return (
        <Drawer
            ...
            description="Các phiên đã lưu, ảnh đối chiếu và độ tin cậy theo từng biển."
        >
                    ...
                    <div className="max-h-[42vh] overflow-y-auto p-3 lg:max-h-none">
                        {loadingJobs ? (
                            ...
                            <EmptyState title="Chưa có phiên">
                                Sau khi xử lý video thành công, phiên và bản ghi
                                nhận dạng sẽ xuất hiện ở đây.
                            </EmptyState>
                        ) : (
                            <div className="space-y-2">
                                {jobs.map((job) => (
                                    <button
                                        ...
                                        type="button"
                                        onClick={() =>
                                            setSelectedJobId(job.session_id)
                                        }
                                        className={cx(
                                    ...
                                    >
                                        ...
                                        <p className="mt-2 data-font truncate text-[11px] text-[var(--color-text-subtle)]">
                                            #{job.session_id}
                                        </p>
                ...
                <section className="min-h-0 bg-[var(--color-bg)] p-4">
                    {error ? (
                        ...
                        <EmptyState title="Chọn một phiên">
                            Danh sách bản ghi sẽ hiển thị ảnh cắt phương tiện,
                            ảnh cắt biển số và độ tin cậy OCR.
                        </EmptyState>
                    ) : vehicles.length === 0 ? (
                        <EmptyState title="Phiên chưa có bản ghi">
                            Không tìm thấy biển số hợp lệ trong phiên này hoặc
