...
class CyclicLRScheduler(BaseLRScheduler):
    ...
    def __init__(self, opts, **kwargs) -> None:
        ...
        if anneal_type not in SUPPORTED_LAST_CYCLES:
            logger.error("Supported anneal types for {} are: {}".format(self.__class__.__name__,
                                                                        SUPPORTED_LAST_CYCLES)
                         )
        if min_lr < end_lr:
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        group = parser.add_argument_group(title="Cyclic LR arguments", description="Cyclic LR arguments")
        group.add_argument('--scheduler.cyclic.min-lr', default=0.1, type=float, help='Min. lr for a cycle')
        group.add_argument('--scheduler.cyclic.last-cycle-end-lr', default=1e-3, type=float,
                           help='End LR for the last cycle')
        group.add_argument('--scheduler.cyclic.total-cycles', default=11, type=int,
                           help='Number of cycles. Default is 10')
        group.add_argument('--scheduler.cyclic.epochs-per-cycle', default=5, type=int,
                           help='Number of epochs per cycle. Default is 5')
        group.add_argument('--scheduler.cyclic.steps', default=None, type=int, nargs='+',
                           help='steps at which LR should be decreased')
        group.add_argument('--scheduler.cyclic.gamma', default=0.5, type=float,
                           help='Factor by which LR should be decreased')
        group.add_argument('--scheduler.cyclic.last-cycle-type', default='linear', type=str,
        ...
        if curr_iter < self.warmup_iterations:
            ...
            if epoch <= self.cyclic_epochs:
                ...
                if self.last_cycle_anneal_type == 'linear':
                    ...
                    curr_lr = self.end_lr + 0.5 * (base_lr - self.end_lr) * (1 + math.cos(math.pi * curr_epoch / period))
                else:
    ...
    def __repr__(self):
        repr_str = '{}(\n \t C={},\n \t C_length={},\n \t C_last={},\n \t Total_Epochs={}, ' \
                   ...
                   '\n \t min_lr={}, \n\t max_lr={}, \n\t end_lr={}\n)'.format(self.__class__.__name__,
                                                                               self.n_cycles, self.cycle_length,
                                                                               self.last_cycle_epochs, self.max_epochs,
...
if __name__ == "__main__":
    opts = None
    sch = CyclicLRScheduler(opts=opts)
