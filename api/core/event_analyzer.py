"""event_analyzer — orchestrates a single mark→analysis job.

Wraps the async ALPR pipeline, translates its event types into
event_* events so the SSE consumer can route them to the right card,
and persists the result to the `events` MongoDB collection.
"""
...
_EVENT_MAP = {
    ...
    "rejected_vehicle": "event_rejected_vehicle",
...
def _persist_event(
    *,
    ...
    """Build a MonitorEvent document and fire-and-forget save it."""
    try:
        from ..database import mongodb
        from ..database.models import EventVehicle, MonitorEvent
        from . import database as core_db

        if not mongodb.is_db_configured():
            return

        # Promote vehicle events into EventVehicle docs, uploading any
        # base64 thumbs to Supabase Storage if present.
        vehicle_docs: list[EventVehicle] = []
        for v in vehicles:
            plate_url = None
            vehicle_url = None
            if v.get("plate_b64"):
            ...
            vehicle_docs.append(EventVehicle(
                track_id=int(v["id"]),
        ...
        fut = asyncio.run_coroutine_threadsafe(mongodb.upsert_event(event), loop)
        fut.add_done_callback(
            lambda f: logger.error("upsert_event failed for %s: %s", event_id, f.exception())
...
def run_event(
    *,
    ...
    def emit_translated(ev: dict) -> None:
        ...
        out = {**ev, "type": new_type, "event_id": event_id}
        if t == "vehicle":
            vehicles.append(ev)
    ...
    try:
        ...
        if ocr_backend == "vietnamese_yolov5":
            ...
            summary = process_frames(
                ...
                emit=emit_translated,
                models=models,
                session_id="",   # we persist via _persist_event, not _record_save
                loop=None,
                timings=timings,
        ...
        if timings is not None:
            logger.info(
                "Event timings event=%s source=%s %s",
                event_id,
                source_type,
